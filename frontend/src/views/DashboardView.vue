...
const getGroupFileChangeMeta = (group: StepGroup) => {
  ...
  for (const step of allSteps) {
    ...
    if (meta.hasChange) {
      ...
      if (meta.isImplementationPlan) {
        result.isImplementationPlan = true;
        result.planSummary = meta.planSummary || '';
      }
    }
  }
  ...
  if (fileTool) {
    try {
      ...
      if (path) {
        ...
        if (args.ArtifactMetadata) {
          meta.isImplementationPlan = args.ArtifactMetadata.RequestFeedback === true;
          meta.planSummary = args.ArtifactMetadata.Summary || '';
        }
      }
    } catch {
                  ...
                  <template v-else-if="group.type === 'response'">
                    <div class="chat-step">
                      <div class="message-bubble assistant-message">
                        ...
                        <div v-if="getGroupFileChangeMeta(group).hasChange">
                          
                          <!-- Implementation Plan Card -->
                          <div v-if="getGroupFileChangeMeta(group).isImplementationPlan" class="implementation-plan-card">
                            <div class="impl-card-header">
                              <FileIcon class="impl-icon" size="18" />
                              <span class="impl-title">Implementation Plan</span>
                            </div>
                            <div class="impl-card-body">
                              <p>{{ getGroupFileChangeMeta(group).planSummary || 'Review the proposed implementation plan.' }}</p>
                              <div class="impl-actions">
                                <AgButton variant="primary" @click="handleSendChat('Proceed', activeConvSteps[activeConvSteps.length - 1].sourceStep)">
                                  Proceed <span class="kbd-shortcut">Ctrl+&crarr;</span>
                                </AgButton>
                                <AgButton variant="secondary" @click.stop="openFileInSidebar(getGroupFileChangeMeta(group).filePath)">
                          ...
                          <!-- Regular Review Bar -->
                          <div v-else class="review-bar-container">
...
/* Implementation Plan Card Styles */
.implementation-plan-card {
  margin-top: 16px;
